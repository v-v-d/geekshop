...
class TestMainappSmoke(TestCase):
    ...
    def test_mainapp_urls(self):
        ...
        response = self.client.get('/products/all/1/')
        self.assertEqual(response.status_code, 200)

        response = self.client.get('/products/category/1/')
        self.assertEqual(response.status_code, 200)

        response = self.client.get('/products/category/1/1/')
        self.assertEqual(response.status_code, 200)

        response = self.client.get('/showroom/')
        self.assertEqual(response.status_code, 200)
        ...
        for category in ProductCategory.objects.all():
            response = self.client.get(f'/products/category/{category.pk}/1/')
            self.assertEqual(response.status_code, 200)
