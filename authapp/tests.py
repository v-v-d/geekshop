...
class TestUserManagement(TestCase):
    ...
    def test_user_login(self):
        ...
        self.assertEqual(response.context['page_title'], 'Interior - main')
        self.assertNotContains(response, self.user.first_name or self.user.username, status_code=200)
        ...
        response = self.client.get('/')
        self.assertContains(response, self.user.first_name or self.user.username, status_code=200)
        self.assertEqual(response.context['user'], self.user)
    ...
    def test_basket_login_redirect(self):
        ...
        self.assertIn('Continue&nbsp;shopping', response.content.decode())
    ...
    def test_user_register(self):
        ...
        self.assertEqual(response.status_code, 200)  # 200 не всегда регистрация, 200 может быть и при неправильно
        # введенных данных
        # self.assertEqual(response.status_code, 302)  # должен быть редирект, чтобы понять, что регистрация прошла
    ...
    def test_user_wrong_register(self):
        ...
        response = self.client.post('/auth/register/', data=new_user_data)
        self.assertEqual(response.status_code, 200)
        self.assertFormError(response, 'register_form', 'age', 'User age must be 18 or more!')
        # self.assertFormError(response, 'form', 'age', 'User age must be 18 or more!') #для CBV 'form'
